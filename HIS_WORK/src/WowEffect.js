 ...
 *   wow.trigger(offer)                       – WOW camera flight + social proof markers
...
export class WowEffect {
  ...
  /**
   * Trigger the WOW-Effect: camera flies to merchant, social proof markers appear.
   * @param {object}            offer
   * @param {[number,number]}   offer.targetLngLat
   * @param {[number,number][]} [offer.socialProofCoords]
  ...
  trigger({ targetLngLat, socialProofCoords = [], flightDuration = 4000 }) {
    ...
    setTimeout(() => {
      ...
      this.#addSocialProofMarkers(socialProofCoords);
   ...
   * @param {{ from: [number,number], to: [number,number] }} params
  ...
  async showRoute({ from, to }) {
    ...
    this.#fitRouteOnMap(route.geometry.coordinates);
  ...
  #fitRouteOnMap(coordinates) {
    const bounds = coordinates.reduce(
      (b, c) => b.extend(c),
      new this.#mgl.LngLatBounds(coordinates[0], coordinates[0])
    );
    this.#map.fitBounds(bounds, { padding: 80, pitch: 45, bearing: 0, duration: 2000 });
  }
